...
namespace Unity
{
	...
	inline Vector3 Vector3Subtract(const Vector3& a, const Vector3& b) {
		return { a.x - b.x, a.y - b.y, a.z - b.z };
	}

	
	inline Vector3 Vector3Normalize(const Vector3& v) { // опять?
		float magnitude = sqrt(v.x * v.x + v.y * v.y + v.z * v.z);
		if (magnitude < 0.0001f) return { 0.0f, 0.0f, 0.0f }; 
